    ...
    internal class OperBD
    {
        ...
        public void Llenartabla(DataGridView dvg, string Fecha_inicio, string fecha_fin)
        {
            try
            {
                using (SqlConnection nuevacon = Conexion.conexion())
                {
                    da = new SqlDataAdapter(string.Format("SELECT TOP 250 ven.cve_pedido AS PEDIDO, ven.cve_siniestro AS SINIESTRO, vend.nombre AS 'VENDEDOR', ven.cve_vendedor AS 'CLAVE VENDEDOR', c.cve_nombre AS CLIENTE, k.nombre AS PIEZA, p.cantidad AS CANTIDAD, ven.fecha_asignacion AS 'FECHA DE ASIGNACIÓN', ven.fecha_promesa AS 'FECHA PROMESA', ven.cve_venta AS 'VENTA',p.cve_pedido AS 'CVE', p.realizo AS 'REALIZADA POR' FROM VENTAS ven LEFT OUTER JOIN PEDIDO p ON ven.cve_venta = p.cve_venta LEFT OUTER JOIN PIEZA k ON p.cve_pieza = k.cve_pieza LEFT OUTER JOIN VALUADOR v ON v.cve_valuador = ven.cve_valuador LEFT OUTER JOIN CLIENTE c ON c.cve_valuador = v.cve_valuador LEFT OUTER JOIN VENDEDOR vend ON ven.cve_vendedor = vend.cve_vendedor WHERE k.nombre != '' AND fecha_asignacion between '{0}' and '{1}' order by ven.fecha_asignacion desc", Fecha_inicio, fecha_fin), nuevacon);
                    nuevacon.Open();
                    dt = new DataTable();
                    da.Fill(dt);
                    dvg.DataSource = dt;
